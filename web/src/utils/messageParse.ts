 ...
 *
 * 解析规则按**真实回复样本**定，不是照 markdown 规范推导。模型回复的共同特征：
 *   - 用无序列表组织内容
 *   - 「**粗体**」独占一行作小节标题
 *   - 条目常带 emoji 图标
 *   - 列表项普遍是「**名称**：描述」的推荐卡形态
...
export function looksLikeTableRow(line: string): boolean {
  ...
  // 至少两个竖线才算「多列」。
  //
  // 不要求以竖线开头或结尾：markdown 表格**允许省略外层竖线**
  // （`车次 | 出发 | 二等座` 是合法写法），要求了反而会漏掉这类表格。
  // 真正的防线是「下一行必须是分隔行」——只按含竖线判定才会误伤正文，
 ...
 * 把条目按时段归组，并**保持原有顺序**。
 *
 * 为什么需要：一天内常有多条活动属于同一时段，若在每条前面都挂一个
 * 「上午/下午/晚上」标签，同一个词会重复出现好几次——真正的内容
