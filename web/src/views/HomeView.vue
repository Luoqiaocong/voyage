...
/*
 * 首屏展开闸门 + 入场滚动揭示都封装在 composable/useHomeReveal.ts：
 * 展开状态、滚动/触摸意图、IntersectionObserver 与生命周期在同一处，
 * 视图只保留「页面入场」和「步骤轮播」两件自身的事。
 */
const { revealed, REVEAL_ID, toggleReveal } = useHomeReveal()

/** 页面入场定时器（首次展开前的 60ms 延时） */
...
/*
 * 展开状态变化时启停步骤轮播。
 * 揭示观察器的注册由 useHomeReveal 内部处理，这里只管轮播。
 */
watch(revealed, (open) => {
  if (!open) {
    stopStepTimer()
