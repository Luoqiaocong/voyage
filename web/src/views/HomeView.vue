 ...
 *
 * 它对齐的是**容器顶端**，而容器（可展开区）顶部有一段内边距，
 * 紧贴其上的引导件因此会被推到视口顶边之外。手动算位置能精确控制落点，
 * 让它停在导航栏下方。
 *
 * ## 关于滚动动画
 *
 ...
 * 目标位置是错的。故等两帧（一帧移除 display、一帧完成布局）再加一小段延时，
 * 让入场过渡走过大部分位移后再计算并滚动。
 */
...
<template>
  ...
  >
    <!-- ============================================================
         1. HERO：一句话描述需求 → 开始规划
         单栏居中：右侧示意卡（车次/天气/预算）与下方「生成结果」区
         内容重复度高，故 Hero 只保留文案与输入框
         （详见 .hero__inner 的样式注释）
         ============================================================ -->
    <section class="hero">
    ...
    >
      <!-- ============================================================
           ...
           故刻意做成**延续**而非并置：
             · 紧接 Hero（间距约 40px），不靠留白分段
             · 引导件是「箭头 + 一行彩色说明」，不是两行文字
      ...
      <section class="section section--join">
        ...
        <!--
          ...
          保持一行短句：两行文字（引导语 + 标题）信息量偏大，会让这一段
          看起来像「新区块的标题」。它的作用只是纯粹的指向 ——
          告诉读者下面的内容是从上面那句话来的。
 ...
 * Hero 内容区。
 *
 * 单栏居中：文案与输入框是唯一的视觉焦点，视线自然落在
 * 「说出你的需求」这个主操作上。
 *
 * 不放「它替我查了什么」示意卡（高铁车次 / 逐日天气 / 预算构成）：
 * 那些内容与下方「生成结果」区重复度高，而首屏最该做的是让人一眼
 ...
 *
 * padding-top 用 40px：这一区的内容只是上一区那句输入的**结果**，
 * 若沿用默认的 88px（叠加 Hero 的下内边距会到 136px 纯空白），
 * 视觉上就「翻页」了，仿佛另起一章。
 */
.section--join {
  padding-top: 40px;
   ...
   * 滚动定位时在顶部留出的余量。
   *
   * 属性必须落在**真正的滚动目标**（下方 .reveal-host）上：加在
   * .section--join 上不会命中。这里是同值的兜底 —— 万一将来有人改用
   * scrollIntoView 或触发浏览器的默认锚点滚动，也会自动避开固定导航栏。
   */
...
/* ============================================================
   区块居中对齐
   ------------------------------------------------------------
   Hero 是居中的单栏构图，其下方各区块必须用同一套对齐方式 ——
   两区对齐方式相反时，页面读起来像几套不同设计拼在一起，
   这是「衔接突兀」最主要的原因。
