 ...
 * 本页只负责「登录」与「注册」两件事。
 *
 * 重置密码在独立页面 /forgot（ForgotView.vue）：三者共用一个组件时
 * 字段与文案会互相渗透，例如注册的「旅行昵称」会混进重置流程。
...
async function handleRegister() {
  ...
  try {
     ...
     *
     * 理由：注册时输入的密码若记错了，当场不会有任何反馈，
     * 等到下次登录才发现时，已经错过了「刚设置完、记忆最新」的纠正时机。
 ...
 * 第三方登录按钮：按品牌色区分。
 *
 * 三个按钮同款（白底灰字）时扫过去分不出哪个是哪个，得逐个读文字；
 * 用品牌色能让人一眼认出，这是第三方登录按钮的通行做法
...
@media (max-width: 720px) {
   ...
   * 移动端场景图高度。
   *
   * 固定高度会在软键盘弹出时把表单挤没 —— 可视区常降到 350px 左右，
   * 用户填验证码时要反复收起键盘才能看到输入框，故按视口高度自适应：
   *   - 普通手机：约 180px，仍保留场景氛围
