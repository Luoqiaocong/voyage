...
背景
----
多值键（preference / dietary / visited_city / companion）的存量数据是一个取值
一行：同一用户同一键有多行，记忆面板上呈现为多张几乎相同的卡片。
目标形态是一行一个键、多项用「、」连接 —— 这也与注入 prompt 时的按键聚合一致。

本脚本把这种旧形态的存量数据折叠成一行一键。对已经符合目标形态的数据
是幂等的（不会重复合并、不会丢值）。
