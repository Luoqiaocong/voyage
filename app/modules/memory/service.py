...
class MemoryService(TransactionMixin):
    ...
    async def _upsert_one(
        ...
        *,
        ...
        #
        # 这些键的真实语义就是「按键聚合」——注入 prompt 时本就按键聚合，
        # 展示也该是一张卡。一行一个键之后：展示是一张卡、注入是一行、
        # 唯一约束也真正生效；若每个取值新增一行，用户会在记忆面板上
        # 看到多张几乎相同的卡。
    ...
    async def _find_loose(self, user_id: int, key: str, value: str) -> UserMemory | None:
        ...
        而重复多来自措辞差异 ——「美食」与「美食。」、「北京」与「北京市」。
        精确匹配挡不住，会各存一条，于是同一个偏好显示两遍。

        多值键是「一行一个键、取值用分隔符挤在一起」，所以比对要逐项做
