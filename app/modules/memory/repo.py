...
冲突消解规则（本期技术重点）
--------------------------
唯一约束建在 (user_id, fact_key, fact_value) 上。注意它只挡得住**完全相同**
的取值，而实测的重复多来自「同一个事实的不同写法」：
  - 「北京、上海、广州」与单独的城市行并存（一个取值塞了多项）
  - 「北京」与「北京市」、「美食」与「美食。」（标点/后缀差异）
  - 「朋友」与「两人同行」（语义等价但措辞不同）

故去重不能只依赖数据库约束，service 层另做了两层处理：
  1. 拆分：多值键的取值先按分隔符摊平（_expand_multi），
     使「北京、上海、广州」能与单城条目比对
  2. 宽松归一：比对时剔除标点、空白与行政区后缀（_canonical）
第三类（语义等价）无法靠规则解决，改从**源头**收敛 ——
提示词要求模型优先使用标准标签，见 schemas.VALUE_ENUMS / RECOMMENDED_VALUES。

分流规则：

1. 重复提取（归一后同一事实）
   → 命中已有行，把 hit_count 加一并刷新时间。
   hit_count 越高说明该事实越稳定，注入 prompt 时可优先使用。

2. 标量键换值（budget_level 从「穷游」变成「舒适」）
   → 三元组不同，但 fact_key 已被占用（标量键至多一行）。
   按「同键覆盖」处理：把旧行的 fact_value 改成新值，
   旧值写入 previous_value 留痕。
