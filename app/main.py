...
# [x] 3. knowledge / RAG 模块实现 —— 场景已明确，并入阶段 3 #13
#
# ─────────── 阶段 2：质量与健壮性 ─────────────────────────────
...
# [ ] 13. RAG：接入私有旅行攻略（下一个开发模块，方案待确认后实施）
#     - 知识库管理：文档上传（txt/md）+ 列表/删除，用户资产归属校验统一 60001
#     - 切块 → DashScope embedding 向量化 → SQLite 新表存储（毕设规模余弦召回足够）
#     - 向量 top-k 召回 → qwen3-rerank 重排 → retrieve_knowledge 工具接入 Agent
# [~] 14. 令牌与注销
