...
async def get_today():
    ...
    **不作为工具暴露给模型**（不在 AGENT_TOOLS 与 tools/__init__ 的导出中）。
    当前日期由 app/core/ai/date_context.py 的中间件直接注入系统提示词：
    日期是服务端每次调用都确切知道的常量，让模型为它跑一次完整的
    「决定调用 → 工具返回 → 组织语言」链路并不划算，
    界面上还会多出一条「获取当前日期」，用户看到会疑惑「这也要查」。

    保留此函数的原因：它仍是一个有用的纯函数（格式化本地日期），单独测试时用得上。
