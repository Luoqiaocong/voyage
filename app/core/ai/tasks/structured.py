...
为什么这样设计（结论均由真实调用实测得出）：
    ...
    schema 由工具定义承载，结构化数据直接落在 tool_calls.arguments 里。
    因此主路径用工具调用；若通道不支持，则回退到「schema 注入提示词 + 本地校验」，
    该回退只依赖最基础的文本补全能力，换任何通道都不会退化。
...
def _render_schema(schema: type[BaseModel], indent: str = "") -> str:
    ...
    必须递归：只列顶层字段时，模型无法判断 daily_plans 的元素结构，
    会把子字段写到错误的层级上。
    """
    lines: list[str] = []
...
async def _extract_via_tool_call(
    ...
    EXTRACT 任务已在 get_task_llm 中关闭思考模式（reasoning_effort="none"），
    ...
    if not isinstance(result, schema):
        raise TypeError(
            f"工具调用返回类型异常：期望 {schema.__name__}，实际 {type(result).__name__}"
        )
...
async def _extract_via_prompt(
    ...
    """回退路径：schema 注入提示词 + 本地 Pydantic 校验（不依赖原生结构化能力）。"""
    llm = get_task_llm(TaskKind.EXTRACT, temperature=temperature)
...
async def extract_structured(
    ...
    *,
    ...
    try:
        ...
        log.error(
            f"[extract_structured] 工具调用不可用，转回退路径: {type(exc).__name__}: {_brief(exc)}"
        )
